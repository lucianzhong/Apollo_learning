...
4.  // 短期路径段查询 (PncMap::GetRouteSegments)
	...
	bool PncMap::GetNearestPointFromRouting(const VehicleState &state,  LaneWaypoint *waypoint) const {
	  ...
	  if (valid_lanes.empty()) {
	    ...
	    std::copy_if(lanes.begin(), lanes.end(), std::back_inserter(valid_lanes), [&](LaneInfoConstPtr ptr) {
	                   return all_lane_ids_.count(ptr->lane().id().id()) > 0;  // 2. 算查询得到的车道和range_lane_ids_或者all_lane_ids_的交集，也就是查找RoutingResponse.road().passage()中的附近的车道
	                 });
	  }
...
6. // 创建车辆当前可行驶区域
	...
	bool PncMap::GetRouteSegments(const VehicleState &vehicle_state, const double backward_length, const double forward_length, std::list<RouteSegments> *const route_segments) {
		...
		// 后接车道处理
		在1中常规的passage中LaneSegment截取结束后，如果router_s仍然小于end_s，就说明车道截取还未结束，还有一段长度end_s - router_s的道路段未被截取，此时passage中的LaneSegment已经全部截取完了，所以需要访问最后一个LaneSegment对应的lane，需要继续截取这条lane的后续部分，如果后续部分长度仍然不够，就需要加入这条lane的后接车道继续截取。
...
LaneSegment2d包含起始点start_pos，结束点end_pos，可以由此计算段方向(end_pos - start_pos)。LaneSegment2d一般是小段。
LaneSegment主要存储段的起始点累积距离start_s，结束点累积距离end_s，没有段的坐标信息。LaneSegment可以存储任意长度的车道。
...
Path::InitWidth可以根据采样点的累计距离，上界和下届MapPathPoint进行插值，使用GetSmoothPoint平滑插值后得到采样点的坐标，并且设置每个采样点的左右车道线距离
/// file in apollo/modules/map/pnc_map/path.cc
void Path::InitWidth() {
  left_width_.clear();
...
8. //由RouteSegments生成的Path包含的成员：

	成员变量或函数														功能

	std::vector<MapPathPoint> path_points_	  					// 原始RouteSegments的MapPathPoint数量，共num_points_个路径点，上图例子中，num_points_=12，一共12个MapPathPoint  // modules/map/pnc_map/path.h
	std::vector<common::math::LineSegment2d> segments_			// MapPathPoint两两构成一个LaneSegment2d，离散化保存RouteSegments，长度为num_points_-1=11		               // modules/map/pnc_map/path.h
	std::vector<double> accumulated_s_							// 每个MapPathPoint的累积距离，长度为num_points_=12		// modules/map/pnc_map/path.h
