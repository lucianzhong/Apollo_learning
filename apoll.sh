...
function release() {
  ...
  for LIB in ${LIBS}; do
    cp -v -a --parent ${LIB} ${APOLLO_RELEASE_DIR}
  done
  mkdir ${APOLLO_RELEASE_DIR}/bazel-bin
  mv -v ${APOLLO_RELEASE_DIR}/bazel-out/local-opt/bin/* ${APOLLO_RELEASE_DIR}/bazel-bin/
  mv -v ${APOLLO_RELEASE_DIR}/bazel-out/local-dbg/bin/* ${APOLLO_RELEASE_DIR}/bazel-bin/
...
function main() {
  ...
  DEFINES="${DEFINES} --copt=-O0 --copt=-g --cxxopt=-O0 --cxxopt=-g "
  DEFINES="${DEFINES} --cxxopt=-std=c++11 "
  ...
  case $cmd in
    ...
    config)
      ...
      ;;
    debug_gpu)
      DEFINES="${DEFINES} --copt=-O0 --copt=-g --cxxopt=-O0 --cxxopt=-g --cxxopt=-DUSE_CAFFE_GPU"
      apollo_build_dbg $@
      ;;
    doc)
      gen_doc
